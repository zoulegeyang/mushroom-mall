<template>
  <div>
    ...
    <div class="form">
      ...
      <van-number-keyboard
        v-model="authCode"
        :show="codekeyboardShow"
        @blur="codekeyboardShow = false"
      />
      <van-number-keyboard
        v-model="password"
        :show="pwdkeyboardShow"
        @blur="pwdkeyboardShow = false"
      />
...
export default {
  ...
  methods: {
    ...
    deletePhone() {
      let tmp = this.phone.slice(0, this.phone.length - 1);
      if (!/^1[3|4|5|8]\d{9}$/.test(this.tmp)) {
        if (tmp.length >= 11) {
          this.telIserr = true;
        }
    ...
    telInput(val) {
      ...
      if (typeof val == "number") {
        let tmp = this.phone + val;
        console.log(tmp);
        if (!/^1[3|4|5|8]\d{9}$/.test(tmp)) {
          if (tmp.length >= 11) {
            this.telIserr = true;
          }
          this.isDisableVer = true;
      ...
      this.$http
        ...
        .then((res) => {
          ...
          if (res.data.code == 200 || res.data.code == 201) {
            ...
            this.userKeeper(res.data.userInfo._doc).then((res) => {
