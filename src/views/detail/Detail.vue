<template>
  <div class="detail">
    ...
    ></detail-tabbar>
    <detail-buy-pop ref="buyPop" :product="payProduct"></detail-buy-pop>
  </div>
</template>
...
import detailBuyPop from "./child/detailBuyPop";
  ...
  methods: {
    ...
    cartTO() {
      ...
      this.cartKeeper(good).then((res) => {
        ...
        // console.log('111')
        let count = this.$store.state.userInfo.cartList.find(
          (item) => item.iid == this.iid
        ).count;
        good.count = count;
        this.$http.post("cart/add", good).then((res) => {
    ...
    selectParams(params) {
      this.$refs.buyPop.show = true;
      this.payProduct = params;
    },
  },
};
</script>
