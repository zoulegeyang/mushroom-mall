<template>
  <div class="edit">
    ...
    <van-cell-group>
      ...
      <van-cell
        title="修改用户名"
        :value="getUserInfo.username ? getUserInfo.username : '还没用户名'"
        @click="open('用户名')"
      />
      <van-cell title="修改手机号" :value="getPhone" @click="open('手机号')" />
      <van-cell
        title="修改密码"
        @click="open('密码')"
        :value="getUserInfo.password ? '已设置' : '未设置'"
      ></van-cell>
      <van-cell
        title="修改邮箱"
        :value="getUserInfo.email ? getUserInfo.email : '还没邮箱'"
        @click="open('邮箱')"
      ></van-cell>
      <van-cell
        title="支付设置"
        @click="open('支付密码')"
        :value="getUserInfo.paypwd ? '已设置' : '未设置'"
      ></van-cell>
      <van-cell
        title="地址管理"
...
export default {
  ...
  computed: {
    ...mapGetters(["getUserInfo", "getPhone"]),
  },
  methods: {
