  ...
  <div class="shopcart">
    <nav-bar class="nav">
      ...
      <div slot="right" class="nav-right" v-show="length > 0">
        ...
        <div @click="editClick" class="edit-text">
          {{ goodTabShow ? "编辑" : "完成" }}
        </div>
      </div>
    </nav-bar>
    <bscroll
      class="scroll"
      ref="bscroll"
      :style="{ bottom: list.length > 0 ? '89px' : '49px' }"
      @pullingUp="loadMore"
      @gundong="xianshi"
      :probe-type="3"
    >
      <div class="have" v-if="list.length > 0">
        <shop-list></shop-list>
      </div>
      <div v-else class="not-have">
        <div class="img">
          ...
          <div>购物车还是空的哦~</div>
          <van-button color="#f08899" to="/home" class="van-button"
            >去逛逛</van-button
          >
        </div>
        <div class="like">
